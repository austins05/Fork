...
class TabulaService {
  ...
  transformJobsToFieldMaps(jobs, detailsCache) {
    return jobs.map((job) => {
      ...
      const cropValue = details?.crop || details?.Crop || '';
      ...
      const result = {
        ...
        area: job.area || 0,  // Requested area
        grossCoverageArea: details?.area_nominal || job.area_nominal || 0,  // Nominal area from API
        status: job.status,
      ...
      // Debug log first job
      if (job.id === 37642) {
        console.log('Job 37642 gross_coverage_area from API:', job.gross_coverage_area);
        console.log('Result grossCoverageArea:', result.grossCoverageArea);
  ...
  async getFieldMapDetails(fieldId) {
    try {
      ...
      return {
        ...
        area: job.area || 0,  // Requested area
        grossCoverageArea: job.area_nominal || 0,  // Nominal area from API
        status: job.status,
        orderNumber: job.order_number || '',
        ...
        crop: job.crop || job.Crop || '',  // Crop field from Tracmap
        productRates: job.product_rates || [],
        requestedUrl: job.requested_url,
        workedUrl: job.worked_url,
        deleted: job.deleted || false,
        color: job.Color || '',
        boundaryColor: job.boundary_color || '',
        urgency: job.urgency || '',
        account: job.account,
